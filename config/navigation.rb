...
SimpleNavigation::Configuration.run do |navigation|
  ...
  navigation.items do |n|
    ...
    n.item :moderation, safe_join([material_symbol('gavel'), t('moderation.title')]), nil, if: lambda {
      ...
      s.item :ng_rules, safe_join([material_symbol('list'), t('admin.ng_rules.title')]), admin_ng_rules_path, highlights_on: %r{/admin/(ng_rules|ng_rule_histories)}, if: -> { current_user.can?(:manage_ng_words) }
      ...
      s.item :special_domains, safe_join([material_symbol('list'), t('admin.special_domains.title')]), admin_special_domains_path, highlights_on: %r{/admin/special_domains}, if: -> { current_user.can?(:manage_federation) }
